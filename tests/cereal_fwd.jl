...
function main()
    ...
    for i in range(1,10,10)
        ...
        # generate emission points and matrix
        ...
        # normal to the configuration hyperplane
        nv = cereal.HodgeV(X[:,1] - X[:,4],X[:,2] - X[:,4],X[:,3] - X[:,4])
        print("Normal vector is: ")
        display(nv)
        print("\n")
        print("Its norm is: $(norm(nv))\n\n")

        if RelTools.is_spacelike(nv)
            print("Normal is spacelike")
        elseif RelTools.is_timelike(nv)
            print("Normal is timelike")
        else
            print("Normal is lightlike")
        end
        print("\n\n")
        ...
        # check if emission points are spacelike separated
        if RelTools.check_constellation(X)
            Xc = locator(X) # pair of solutions
            valid_1 = RelTools.check_solution(Xc[1], X) # check solution #1
            valid_2 = RelTools.check_solution(Xc[2], X) # check solution #2
